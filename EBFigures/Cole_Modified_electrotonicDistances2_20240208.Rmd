...
```{r}
# #read out the inputs to the neuron you select
...
partner_info = rbind(partner_info, neuprint_search((".*LAL.*")))
partner_info = rbind(partner_info, neuprint_search((".*DN.*")))
partner_info = rbind(partner_info, neuprint_search((".*PS.*")))
partner_info = rbind(partner_info, neuprint_search((".*VES.*")))
partner_info = rbind(partner_info, neuprint_search((".*AOTU.*")))
partner_info = rbind(partner_info, neuprint_search((".*IB.*")))
partner_info = rbind(partner_info, neuprint_search((".*PLP.*")))
partner_info = rbind(partner_info, neuprint_search((".*MBON.*")))
partner_info = rbind(partner_info, neuprint_search((".*CRE.*")))
partner_info = rbind(partner_info, neuprint_search((".*LT.*")))
partner_info = rbind(partner_info, neuprint_search(".*DNa02_R*"))

	

(partner_info)
sort(unique(partner_info$type))
# 
...
```{r read in neurons}
#I still don't understand how to get this cell working properly when my cell type is DNa02
...
# 
# if(nronType == 'DNa02'){
#   npfl1 = grep('NPFL1-I',nron_info$name)
#   glno = grep('GLNO',nron_info$name)
#   lcnop = grep('LCNOp',nron_info$name)
#   lcnopm = grep('LCNOpm',nron_info$name)
#   lno1 = grep('LNO1',nron_info$name)
#   lno2 = grep('LNO2',nron_info$name)
#   lno3 = grep('LNO3',nron_info$name)
#   lnoa = grep('LNOa',nron_info$name)
#   mdn = grep('MDN',nron_info$name)
#   exclude = unique(c(npfl1, glno, lcnop, lcnopm, lno1, lno2, lno3, lnoa, mdn))
# } else {
#   exclude = c()
# }
...
```{r}
if(savePlot){
  pdf(paste('cole_plots_02/example', nronType, '_RP.pdf', sep=""))
}
...
# partner_info = rbind(partner_info, neuprint_search(".*DNa02*"))
# ms = c('ER1_a','ER1_b')
...
modalityCDFPlot = ggplot(filter(synapsesPre, bodyid == nron_info_kept$bodyid[egNum], modality %in% c('all')), aes(x = normEDist, color = modality)) +
    stat_ecdf(geom = "step") +
...
if(savePlot){
ggsave(paste('modalityCDF_', nronType, '_ED.pdf', sep = ""), plot = modalityCDFPlot, device='pdf', path = "cole_plots_02")
}
...
modalityCDFPlot = ggplot(filter(synapsesPre, bodyid == nron_info_kept$bodyid[egNum], modality %in% c('all')), aes(x = distToRoot, color = modality)) +
    stat_ecdf(geom = "step") +
    scale_colour_manual(values = modalityCMap) +
    theme_paper() +
...
if(savePlot){
ggsave(paste('modalityCDF_', nronType, '_RD.pdf', sep = ""), plot = modalityCDFPlot, device='pdf', path = "cole_plots_02")
}
...
```{r}
medOfEDist_nronType = filter(synapses, prepost == 1, modality %in% c('PFL3', 'LAL', 'Vest','Posterior_Slope', 'DNa03')) %>% 
...
medOfEDist = filter(synapses, prepost == 1) %>% group_by(bodyid, modality) %>%
            summarize(med = median(normEDist)) %>%
            filter(modality %in% c('PFL3', 'LAL', 'Vest','Posterior_Slope', 'DNa03')) %>%
            group_by(bodyid) %>%
...
```{r}
  ...
  geom_line(aes(x = factor(modality, levels = c('PFL3','DNa03','Vest', 'LAL','Posterior_Slope')), y = med,  group = factor(bodyid), color = factor(eg)), position = position_dodge(0.5), size = 0.5) +
  geom_point(aes(x = factor(modality, levels = c('PFL3','DNa03','Vest', 'LAL','Posterior_Slope')), y = med, fill = modality, group = factor(bodyid)), color = 'black', pch = 21, position = position_dodge(0.5), size = 3, alpha = 0.75) +
  theme_paper() +
  guides(fill = 'legend') +
...
if(savePlot){
ggsave(paste('medianDists_hive_jitter_', nronType, '_ED.pdf', sep = ""), plot = medHivePlot, device='pdf', path = "cole_plots_02")
}
...
medOfDist = filter(synapses, prepost == 1) %>% group_by(bodyid, modality) %>%
  ...
  geom_line(aes(x = factor(modality, levels = c('PFL3', 'LAL', 'Vest','Posterior_Slope', 'DNa03')), y = med,  group = factor(bodyid), color = factor(eg)), position = position_dodge(0.5), size = 0.5) +
  geom_point(aes(x = factor(modality, levels = c('PFL3', 'LAL', 'Vest','Posterior_Slope', 'DNa03')), y = med, fill = modality, group = factor(bodyid)), color = 'black', pch = 21, position = position_dodge(0.5), size = 3, alpha = 0.75) +
  theme_paper() +
  guides(fill = 'legend') +
  scale_fill_manual(values = modalityCMap) +
  scale_color_manual(values = c('lightgray','black',modalityCMap)) +
...
if(savePlot){
  ggsave(paste('medianDists_hive_jitter_', nronType, '_RD.pdf', sep = ""), plot = medHivePlot, device='pdf', path='cole_plots_02')
}
...
if(savePlot){
ggsave(paste('medianDists_hive_jitter_', nronType, '_motorED.pdf', sep = ""), plot = medHivePlot, device='pdf', path = "cole_plots_02")
}
...
if(savePlot){
ggsave(paste('medianDists_hive_jitter_', nronType, '_msED.pdf', sep = ""), plot = medHivePlot, device='pdf', path = "cole_plots_02")
}
...
if(savePlot){
ggsave(paste('medianDists_hive_jitter_', nronType, '_ivED.pdf', sep = ""), plot = medHivePlot, device='pdf', path = "cole_plots_02")
}
...
if(savePlot){
ggsave(paste('medianDists_hive_jitter_', nronType, '_cvED.pdf', sep = ""), plot = medHivePlot, device='pdf', path = "cole_plots_02")
}
...
if(savePlot){
ggsave(paste('medianDists_hive_jitter_', nronType, '_sleepED.pdf', sep = ""), plot = medHivePlot, device='pdf', path = "cole_plots_02")
}
...
if(savePlot){
ggsave(paste('rankOrdering_', nronType, '_ED.pdf', sep = ""), plot = rankOrderingPlot, device='pdf', path = "cole_plots_02")
}
```
...
if(savePlot){
ggsave(paste('distDistance_', nronType, '_ED.pdf', sep = ""), plot = distDistancePlot, device='pdf', path = "cole_plots_02")
}
```
...
if(savePlot){
ggsave(paste('depthVsRadial_', nronType, '_ED.pdf', sep = ""), plot = depthVRadDensPlot, device='pdf', path = "cole_plots_02", scale = 1.5, width = 7, height = 7, units ="cm", dpi = 600, limitsize = TRUE)
}
...
if (savePlot){
ggsave(paste('depthVsRadial_', nronType, '_RD.pdf', sep = ""), plot = depthVRadDensPlot, path = "cole_plots_02", device='pdf', scale = 1.5, width = 7, height = 7, units ="cm", dpi = 600, limitsize = TRUE)
}
...
if(savePlot){
ggsave(paste('depthVsRadial_modalities_',nronType,'.pdf', sep = ""), plot = depthVRadMod, device='pdf', path = "cole_plots_02", scale = 1.5, width = 7, height = 7, units ="cm", dpi = 600, limitsize = TRUE)
}
...
if(savePlot){
ggsave(paste('depthVsRadial_modalities_',nronType,'_motor.pdf'), plot = depthVRadMod, device='pdf', path = "cole_plots_02", scale = 1.5, width = 7, height = 7, units ="cm", dpi = 600, limitsize = TRUE)
}
...
if(savePlot){
ggsave(paste('depthVsRadial_modalities',nronType,'_ms.pdf'), plot = depthVRadMod, device='pdf', path = "cole_plots_02", scale = 1.5, width = 7, height = 7, units ="cm", dpi = 600, limitsize = TRUE)
}
...
if(savePlot){
ggsave(paste('depthVsRadial_modalities',nronType,'_iv.pdf'), plot = depthVRadMod, device='pdf', path = "cole_plots_02", scale = 1.5, width = 7, height = 7, units ="cm", dpi = 600, limitsize = TRUE)
}
...
if(savePlot){
ggsave(paste('depthVsRadial_modalities',nronType,'_cv.pdf'), plot = depthVRadMod, device='pdf', path = "cole_plots_02", scale = 1.5, width = 7, height = 7, units ="cm", dpi = 600, limitsize = TRUE)
}
...
if(savePlot){
ggsave(paste('depthVsRadial_modalities',nronType,'_sleep.pdf'), plot = depthVRadMod, device='pdf', path = "cole_plots_02", scale = 1.5, width = 7, height = 7, units ="cm", dpi = 600, limitsize = TRUE)
}
