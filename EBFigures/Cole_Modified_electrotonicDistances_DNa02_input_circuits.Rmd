...
# 
# # Create the plot with swarm plots for each modality
# plot_swarm <- ggplot(new_df, aes(x = modality, y = normEDist, color = modality)) +
#   geom_beeswarm(cex = .75) +
#   stat_summary(fun.data = "median_hilow", geom = "crossbar", width = 0.5, fatten = 2, color = "black") +
#   stat_summary(fun.data = "mean_cl_boot", geom = "errorbar", width = 0.2, color = "black") +
#   scale_color_manual(values = c("ACh" = '#32CD32', "GABA"  = '#FFA500', "Glutamate" = '#FF00FF')) +  # Color mapping for lines
...
```{r}
colnames(DNa02_conn_info)

#Determine and plot the Strahler order of the branches

sg <- segmentgraph(DNa02_nron)
strahler_info <- strahler_order(DNa02_nron)
...
strahler_info_w_conns <- strahler_order(DNa02_nron)
...
medOfDist = filter(synapses, prepost == 1) %>% group_by(bodyid, modality) %>%
            ...
            filter(modality %in% c('GABA', 'Glutamate', 'ACh')) %>%
            group_by(bodyid) %>%
            mutate(rankOrder = rank(med)) %>%
            ungroup() %>% 
            mutate(eg = case_when(
  ...
  geom_line(aes(x = factor(modality, levels = c('GABA', 'Glutamate', 'ACh')), y = med,  group = factor(bodyid), color = factor(eg)), position = position_dodge(0.5), size = 0.5) +
  geom_point(aes(x = factor(modality, levels = c('GABA', 'Glutamate', 'ACh')), y = med, fill = modality, group = factor(bodyid)), color = 'black', pch = 21, position = position_dodge(0.5), size = 3, alpha = 0.75) +
  theme_paper() +
  guides(fill = 'legend') +
  scale_fill_manual(values = modalityCMap) +
  scale_color_manual(values = c('lightgray','black',modalityCMap)) +
...
if(savePlot){
  ggsave(paste('medianDists_hive_jitter_', nronType, '_RD.pdf', sep = ""), plot = medHivePlot, device='pdf', path='NT_Inputs_to_DNa02')
}
...
medHivePlot = ggplot(filter(medOfEDist_nronType, partnerType %in% GABA)) +
...
if(savePlot){
ggsave(paste('medianDists_hive_jitter_', nronType, '_GABA_ED.pdf', sep = ""), plot = medHivePlot, device='pdf', path = "NT_Inputs_to_DNa02")
}

# Figure 12 supplement 2B
medHivePlot = ggplot(filter(medOfEDist_nronType, partnerType %in% Glutamate)) +
  geom_line(aes(x = factor(partnerType), y = med,  group = factor(bodyid), color = factor(eg)), position = position_dodge(0.5), size = 0.5) +
  geom_point(aes(x = factor(partnerType), y = med, fill = partnerType, group = factor(bodyid)), color = 'black', pch = 21, position = position_dodge(0.5), size = 3, alpha = 0.75) +
  theme_paper() +
  guides(fill = 'legend') +
...
if(savePlot){
ggsave(paste('medianDists_hive_jitter_', nronType, '_Glutamate_ED.pdf', sep = ""), plot = medHivePlot, device='pdf', path = "NT_Inputs_to_DNa02")
}


medHivePlot = ggplot(filter(medOfEDist_nronType, partnerType %in% Acetylcholine)) +
  geom_line(aes(x = factor(partnerType), y = med,  group = factor(bodyid), color = factor(eg)), position = position_dodge(0.5), size = 0.5) +
  geom_point(aes(x = factor(partnerType), y = med, fill = partnerType, group = factor(bodyid)), color = 'black', pch = 21, position = position_dodge(0.5), size = 3, alpha = 0.75) +
  theme_paper() +
  guides(fill = 'legend') +
...
if(savePlot){
ggsave(paste('medianDists_hive_jitter_', nronType, '_ACh_ED.pdf', sep = ""), plot = medHivePlot, device='pdf', path = "NT_Inputs_to_DNa02")
}
...
if(savePlot){
ggsave(paste('rankOrdering_', nronType, '_ED.pdf', sep = ""), plot = rankOrderingPlot, device='pdf', path = "NT_Inputs_to_DNa02")
}
```
...
if(savePlot){
ggsave(paste('distDistance_', nronType, '_ED.pdf', sep = ""), plot = distDistancePlot, device='pdf', path = "NT_Inputs_to_DNa02")
}
```
...
if(savePlot){
ggsave(paste('depthVsRadial_', nronType, '_ED.pdf', sep = ""), plot = depthVRadDensPlot, device='pdf', path = "NT_Inputs_to_DNa02", scale = 1.5, width = 7, height = 7, units ="cm", dpi = 600, limitsize = TRUE)
}
...
if (savePlot){
ggsave(paste('depthVsRadial_', nronType, '_RD.pdf', sep = ""), plot = depthVRadDensPlot, path = "NT_Inputs_to_DNa02", device='pdf', scale = 1.5, width = 7, height = 7, units ="cm", dpi = 600, limitsize = TRUE)
}
...
```{r}
depthVRadMod = ggplot(filter(synapses, prepost == 1, modality %in% c('GABA', 'Glutamate', 'ACh'))) +
...
if(savePlot){
ggsave(paste('depthVsRadial_modalities_',nronType,'.pdf', sep = ""), plot = depthVRadMod, device='pdf', path = "NT_Inputs_to_DNa02", scale = 1.5, width = 7, height = 7, units ="cm", dpi = 600, limitsize = TRUE)
}
...
depthVRadMod = ggplot(filter(synapses, prepost == 1, modality %in% c('GABA'))) +
...
if(savePlot){
ggsave(paste('depthVsRadial_modalities_',nronType,'_GABA.pdf'), plot = depthVRadMod, device='pdf', path = "NT_Inputs_to_DNa02", scale = 1.5, width = 7, height = 7, units ="cm", dpi = 600, limitsize = TRUE)
}

depthVRadMod = ggplot(filter(synapses, prepost == 1, modality %in% c('Glutamate'))) +
  stat_density_2d(aes(y = radial, x = depth, fill = modality, alpha = stat(nlevel)), bins = 40, contour_var = 'ndensity', geom = 'polygon')+
  geom_path(data=outline, aes(x=c1, y=c2), size = 0.5) +
...
if(savePlot){
ggsave(paste('depthVsRadial_modalities',nronType,'_Glutamate.pdf'), plot = depthVRadMod, device='pdf', path = "NT_Inputs_to_DNa02", scale = 1.5, width = 7, height = 7, units ="cm", dpi = 600, limitsize = TRUE)
}

depthVRadMod = ggplot(filter(synapses, prepost == 1, modality %in% c('Acetylcholine'))) +
  stat_density_2d(aes(y = radial, x = depth, fill = modality, alpha = stat(nlevel)), bins = 40, contour_var = 'ndensity', geom = 'polygon')+
  geom_path(data=outline, aes(x=c1, y=c2), size = 0.5) +
...
if(savePlot){
ggsave(paste('depthVsRadial_modalities',nronType,'_ACh.pdf'), plot = depthVRadMod, device='pdf', path = "NT_Inputs_to_DNa02", scale = 1.5, width = 7, height = 7, units ="cm", dpi = 600, limitsize = TRUE)
}
# 
...
# 
# if(savePlot){
# ggsave(paste('depthVsRadial_modalities',nronType,'_VES.pdf'), plot = depthVRadMod, device='pdf', path = "NT_Inputs_to_DNa02", scale = 1.5, width = 7, height = 7, units ="cm", dpi = 600, limitsize = TRUE)
# }
# 
...
```
```{r}
dna02 = neuprint_read_neurons(1140245595, heal = TRUE, soma = TRUE)
```
